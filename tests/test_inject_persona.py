...
``inject_persona.py`` is a standalone script shipped inside ``persona-*``
plugins (not part of the ``scripts`` package), so it is loaded directly from
its file path rather than imported as a module path.
"""
